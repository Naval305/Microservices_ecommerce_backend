...
def set_user_active_status(user_id, is_active: bool):
    try:
        ...
        logger.warning("Redis unavailable while caching active status for user %s: %s", user_id, exc)
...
def get_cached_user_active_status(user_id):
    try:
        ...
        logger.warning("Redis unavailable while reading cached active status for user %s: %s", user_id, exc)
